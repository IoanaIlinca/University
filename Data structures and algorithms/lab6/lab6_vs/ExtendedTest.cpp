...
void testIteratorSteps(SortedMultiMap& m) {
	...
	for (int i = 0; i < count / 2; i++) {
		TElem e = smmi.getCurrent();
		////cout << i << " current: " << e.first << ' ' << e.second << '\n';
		smmi.next();
	}
	smmi.first();
	int c = 0;
...
void testRelation(Relation r) {
	...
	while (smit.valid()) {
		...
		////cout << " f: " << smit.getCurrent().first << " s: " << smit.getCurrent().second;
...
void testIterator(Relation r) {
	...
	for (int i = 0; i < n; i++) {
      ...
      smm.add(keys[i], 100);
	  //cout << "added:" << keys[i] << " 100\n";
	  if (keys[i]%2==0)	{
		smm.add(keys[i], 200);
		//cout << "added:" << keys[i] << " 200\n";
	  }
	}
	//cout << "I got here!5\n";
	testIteratorSteps(smm);
