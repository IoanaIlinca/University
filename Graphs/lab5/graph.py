...
class Graph(object):
    ...
    def addEdge(self, source, destination, cost):
        ...
        if self.checkIsEdge(source, destination) == False and source != destination:
            ...
            self.graph.append([source, destination, cost])
...
# (uses union by rank) 
    ...
    def KruskalMST(self): 
        ...
        while e < len(self.graph) - 1 and i < len(self.graph) : 

                # Step 2: Pick the smallest edge and increment 
                        # the index for next iteration 
            
            u,v,w = self.graph[i] 
            print("Current edge: %d, %d, %d" % (u,v,w))
            i = i + 1
        ...
        print("Cost of Minimum spanning tree: %d" % cost)

    def checkVertexCover(self, lis):
        for edge in self.graph:
            if (edge[0] not in lis) and (edge[1] not in lis):
                return False
        return True 
    
    def Combinations(self, lis, k):
        if self.__vertexCover != []:
            return
        if (len(lis) == k):
            if (self.checkVertexCover(lis) == True and self.__vertexCover == []):
                self.__vertexCover = lis
    ...
    def vertexCover(self):
        for k in range(1, len(self.__graphIn) + 1):
            lis = []
            #print(k)
            self.Combinations(lis, k)
            if self.__vertexCover != []:
        ...
        for node in self.__vertexCover:
            print(node)
