...
{
    //printf("I will destroy a profile");
    if (profil == NULL)
        return;
...
Profile* Copy(Profile* profil)
{
    ...
    newP->placeOfBirth = (char*)malloc((strlen(profil->placeOfBirth) + 1) * sizeof(char));
    newP->psychologicalProfile = (char*)malloc((strlen(profil->psychologicalProfile) + 1) * sizeof(char));
    strcpy(newP->placeOfBirth, profil->placeOfBirth);
    strcpy(newP->psychologicalProfile, profil->psychologicalProfile);
    newP->profileIdNumber = profil->profileIdNumber;
    newP->yearsOfRecordedService = profil->yearsOfRecordedService;
    return newP;
}
