...
GUI::GUI(Service<Vegetable>& s) : serv{ s }
{
	this->initGUI();
	this->populateList();
...
void GUI::initGUI()
{
	...
	evidenceDetailsLayout->addRow("Vegetable", this->vegetableLineEdit);
	...
	secondLayout->addRow("Parts", this->partsLineEdit);
...
void GUI::populateList()
{
	...
	vector<string> veggies = this->serv.GetFamilies();
	for (auto& p : veggies)
	{
		this->evidenceListWidget->addItem(QString::fromStdString(p));
		
	}
		
}

void GUI::populateListVeggies(vector<Vegetable> veggiesFromFamily)
{
	...
	QObject::connect(this->evidenceListWidget, &QListWidget::itemSelectionChanged, [this]()
		{
			...
			vector<Vegetable> veggiesFromFamily, elements = this->serv.GetElements();
...
void GUI::Search()
{
	...
	if (indexHighlight == -1)
	{
		QMessageBox::critical(this, "Error", "No such vegetable");
		return;
	}
