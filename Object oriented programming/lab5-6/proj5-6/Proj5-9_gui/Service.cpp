...
bool ServiceB::SaveIdToMyList(string id)
{
	...
	unique_ptr<RepoB> ptr = make_unique<RepoB>(repositoryMyList);
	unique_ptr<ActionSave> thisAction = make_unique<ActionSave>(move(ptr), piece);
	actionsUndoB.emplace(move(thisAction));
	return true;
}

bool ServiceB::UnsaveIdToMyList(string id)
{
	PieceOfEvidence piece = ServiceElement(id);
	repositoryMyList.UnsavePiece(piece);
	return true;
}
...
vector<PieceOfEvidence> ServiceB::GetMyList()
{
	return repositoryMyList.ReadFromFile();
}

RepoB& ServiceB::GetRepoMyList()
{
	return repositoryMyList;
}

string ServiceB::UndoB()
{
	if (actionsUndoB.empty() == true)
		return "No actions to undo";
	unique_ptr<Action> thisAction = move(actionsUndoB.top());
	thisAction->executeUndo();
	actionsRedoB.emplace(move(thisAction));
	actionsUndoB.pop();
	return "";
}

string ServiceB::RedoB()
{
	if (actionsRedoB.empty() == true)
		return "No actions to redo";
	unique_ptr<Action> thisAction = move(actionsRedoB.top());
	thisAction->executeRedo();
	actionsUndoB.emplace(move(thisAction));
	actionsRedoB.pop();
	return "";
